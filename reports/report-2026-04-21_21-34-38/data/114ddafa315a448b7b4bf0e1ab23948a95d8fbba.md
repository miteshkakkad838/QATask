...
Locator: locator('#inventory-list')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#inventory-list')
...
```ts
  ...
  7  | 
  8  | test('Login Test - SauceDemo', async ({ page }) => {
  9  | 
  ...
  14 | 
> 15 |     await expect(page.locator('#inventory-list')).toBeVisible(); //assertion
     |                                                   ^ Error: expect(locator).toBeVisible() failed
  16 | });
```